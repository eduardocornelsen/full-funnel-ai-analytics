...
SELECT
    ...
    -- Ad-platform conversions (Google + Meta clicks → purchase): campaign CVR denominator
    COALESCE(a.ad_conversions, 0)          AS total_ad_conversions,
    -- GA4 session-level conversions: canonical cross-channel CVR (Session CVR)
    COALESCE(s.ga4_conversions, 0)         AS total_conversions,
    COALESCE(s.total_sessions, 0)          AS ga4_total_sessions,
    COALESCE(s.total_engaged_sessions, 0)  AS ga4_engaged_sessions,
